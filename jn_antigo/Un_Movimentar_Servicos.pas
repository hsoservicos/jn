...
unit Un_Movimentar_Servicos;

interface

uses
   Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms, Dialogs, StdCtrls, Buttons, DB, ExtCtrls;

type
  T_Movimentar_Servicos = class(TForm)
    gbOS: TGroupBox;
    gbCliente: TGroupBox;
    lblCliente: TLabel;
...
var
   _Movimentar_Servicos: T_Movimentar_Servicos;
...
procedure T_Movimentar_Servicos.FormShow(Sender: TObject);
begin
   LimpaDados;
   _dm.qr11.Close;
...
procedure T_Movimentar_Servicos.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
   Case Key Of
      VK_ESCAPE : Close;
      VK_RETURN :
...
procedure T_Movimentar_Servicos.sbSairClick(Sender: TObject);
begin
   Close;
end;

procedure T_Movimentar_Servicos.LimpaDados;
begin
   edtOs.Text := '';
   cbSituacao.Text := '';
...
procedure T_Movimentar_Servicos.btnAlterarClick(Sender: TObject);
begin
   If edtOs.Text <> '' Then
   Begin
      _dm.qr07.Close;
      ...
      Begin
         ...
         cbSituacao.ItemIndex   := _Main.stlSit.IndexOf( _dm.qr07Situacao.AsString ); //
         ...
         edtOrcamento.Text      := _dm.qr07ValorOrcamento.AsString;
         cbSituacao.SetFocus;
      End;
   End
   Else
...
procedure T_Movimentar_Servicos.edtObsExit(Sender: TObject);
begin
   If MessageDlg( 'Confirma registra da Alteração ?', mtConfirmation, [ mbYes, mbNo ], 0 ) = mrYes Then
   Begin
//      cSQL := 'update SERVICOS set Situacao = ' + _Main.stlSit.Strings[ cbSituacao.ItemIndex ] +
//              ', ValorOrcamento = ' + edtOrcamento.Text +
...
//              ', Obs = "' + UpperCase( edtObs.Text ) + '" where CodArea = "' + edtOs.Text + '"';
      ...
      _dm.qr07.Edit;
      _dm.qr07.FieldByName( 'Situacao' ).AsString         := _Main.stlSit.Strings[ cbSituacao.ItemIndex ];
      _dm.qr07.FieldByName( 'ValorOrcamento' ).AsString   := edtOrcamento.Text;
